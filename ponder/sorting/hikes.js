...
const otherHikes = ["Yellowstone", "yellowstone", "YELLOWSTONE", "\\YELLOWSTONE", "!Yellowstone", "Goldbug Hotsprings", "Teton"]

console.log(otherHikes);

console.log(otherHikes.sort());
...
const people = [{ name: "James" }, { name: "Bob" }, { name: "Josh" }];
...
searchButton.addEventListener("click", e => {
    ...
    const sortHikes = filterHikes.sort(sortDifficulty)
    console.log(sortHikes)

    hikeContainer.innerHTML = "";

    sortHikes.forEach(function(hike) {
        renderHike(hike);
    });
    ...
    const hikeTemplate = `
        <div class="hike-card">
            <div class="hike-content">
                <h2>${hike.name}</h2>
                <div class="hike-tags">
                    ${templateTags(hike.tags)}
                </div>
            <p>${hike.description}</p>
            <p>${difficultyTemplate(difficultyTemplate(hike.difficulty))}</p>
            </div>
        </div>`

        hikeContainer.innerHTML += hikeTemplate
}
