...
const recipes = [
    {
        ...
        image: './sweet_potato_waffles.png',
        recipeIngredient: [
            '2 separated eggs',
            '1/4 C Oil',
            '1/4 tsp salt',
    ...
    },
    {
        ...
        image: './excalopes_de_poulet_a_la_creme.png',
        recipeIngredient: [
    ...
    },
    {
        ...
        image: './german_goose_berry_cake.png',
        recipeIngredient: [
            'For the Cake Base:',
...
const recipeContainer = document.getElementById("recipeContainer"); // Make sure this exists in your HTML

// Initial render - show a random recipe or all recipes
...
function renderRecipe(recipe) {
    ...
    <div class="recipe">
        <img class="big" src="${recipe.image}" alt="${recipe.name} image">
        <div class="recipe_description">
            <label class="recipe_type">${recipe.tags}</label>
            <h2>${recipe.name.toUpperCase()}</h2>
            <span class="rating" role="img" aria-label="Rating: ${recipe.rating} out of 5 stars">
                ${generateRatingStars(recipe.rating)}
            </span>
            <p class="larger_device">${recipe.description}</p>
            <div class="tags">
                ${templateTags(recipe.tags)}
            </div>
        </div>
    </div>`;
...
searchButton.addEventListener("click", e => {
    ...
    console.log("Filtered recipes:", filterRecipes.length);

    // Sort recipes by name
    const sortRecipes = filterRecipes.sort((a, b) => {
        return a.name.localeCompare(b.name);
    });

    console.log("Sorted recipes:", sortRecipes.length);

    // Render the filtered and sorted recipes
    renderAllRecipes(sortRecipes);
});

// Optional: Add search on input (real-time search)
